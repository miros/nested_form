var NestedForm = {
  addFieldsHandler: function(eventData, event) {
    ...
    var context_node_finder = $(this).data('context_finder');
    context_node_finder = typeof(context_node_finder) == 'function' ? context_node_finder : NestedForm.contextNodeFinder;

    // Make the context correct by replacing new_<parents> with the generated ID
    // of each of the parent objects
    var context_node = context_node_finder.call(this);
    var context = (context_node.find('input:first').attr('name') || '').replace(new RegExp('\[[a-z]+\]$'), '');
    ...
    $(this).closest("form").trigger('nested:fieldAdded', [new_id, assoc]);
  ...
  contextNodeFinder: function() {
    return $(this).closest('.fields');
  },
